...
async def measure_source_roundtrip(source, query_text: str = "nike", samples: int = 3):
    ...
    from .sources.base import SearchQuery, SourceError
...
async def run(keywords: list[Keyword], hosts: list[str] | None = None) -> dict[str, Any]:
    hosts = hosts or ["api.mercari.jp", "auctions.yahoo.co.jp", "api.telegram.org"]
    ...
    simulator = SimulatorSource(seed=7, new_per_second=400.0, noise_ratio=0.2)
