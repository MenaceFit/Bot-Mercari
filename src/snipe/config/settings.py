...
atterrir dans `config.yaml`**. Le fichier YAML est fait pour être partagé,
...
class Settings:
    ...
    def apply_env(self) -> None:
        ...
        if value := os.getenv("SNIPE_LOG_LEVEL"):
            self.log_level = value.strip().upper()
        if value := os.getenv("SNIPE_BUDGET"):
            try:
                self.scanner.budget_per_second = float(value)
            except ValueError:
                log.warning("SNIPE_BUDGET invalide : %r", value)
