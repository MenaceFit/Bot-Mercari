#!/usr/bin/env bash
# Radar Mercari — lanceur Linux / macOS
set -uo pipefail
cd "$(dirname "$0")"
...
[[ -f radar.yaml ]] || { echo "  [i] Création de radar.yaml…"; "$PY" -m radar init; echo; }
...
code=$?
echo
[[ $code -ne 0 ]] && echo "  Arrêt avec le code $code. Diagnostic : $PY -m radar doctor"
exit $code
