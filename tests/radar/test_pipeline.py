...
from buyee_radar.adapters.base import AdapterError, SearchResult
from buyee_radar.core.circuit_breaker import CircuitBreaker, State
from buyee_radar.core.currency import CurrencyConverter
from buyee_radar.core.event_bus import EventBus
from buyee_radar.core.keywords import Keyword
from buyee_radar.core.matcher import FilterEngine, GlobalFilters
from buyee_radar.core.scanner import Scanner
from buyee_radar.core.scheduler import Scheduler
from buyee_radar.core.scoring import ScoringConfig, ScoringEngine, tier_of
from buyee_radar.notifications.dispatcher import NotificationHub
from buyee_radar.storage.database import Database
...
class TestPipeline:
    ...
    async def test_unsupported_adapters_are_not_scheduled(self, database, keywords):
        from buyee_radar.adapters.buyee_html import MARKETPLACES, BuyeeAdapter

        adapter = BuyeeAdapter(MARKETPLACES["rakuten"])
        hub = NotificationHub([])
        scanner = build({"rakuten": adapter}, keywords, database, hub)
        await scanner.start()
        assert len(scanner.scheduler.tasks) == 0
        await scanner.stop()
...
class TestConfigIsolation:
    ...
    def test_radar_does_not_share_snipe_config_file(self):
        from buyee_radar.config.loader import DEFAULT_PATH as radar_path
